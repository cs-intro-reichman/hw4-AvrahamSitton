public class KeywordsDetector {
    ...
    public static void detectAndPrint(String[] sentences, String[] keywords) {
        for (String sentence : sentences) {
            for (String keyword : keywords) {
                if (containsIgnoreCase(sentence, keyword)) {
    ...
    public static boolean containsIgnoreCase(String sentence, String keyword) {
        ...
        for (int i = 0; i <= sentenceLength - keywordLength; i++) {
            boolean match = true;
            for (int j = 0; j < keywordLength; j++) {
                char sentenceChar = sentence.charAt(i + j);
                char keywordChar = keyword.charAt(j);

                if (sentenceChar >= 'A' && sentenceChar <= 'Z') {
                    sentenceChar += 32;
                }
